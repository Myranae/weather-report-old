...
const changeBgImg = () => {
    if (state.temp >= 80) {
        ...
        document.body.style.backgroundImage = `url(${dict[fiftyDegrees]})`;
    } else if (state.temp >= 40) {
        document.body.style.backgroundImage = `url(${dict[fortyDegrees]})`;
    } else {
        document.body.style.backgroundImage = `url(${dict[elseImg]})`;
    }
};
